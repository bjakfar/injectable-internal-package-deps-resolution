...
class InjectableHolisticGenerator extends GeneratorForAnnotation<PackageDependenciesLoader> {
  ...
  Future<Map<String, List<LibraryElement>>> _resolveLibraryElementsInMicroModules(Iterable<String> microModules) async {
    final microModuleToLibraryElements = <String, List<LibraryElement>>{};
    for (final microModule in microModules) {
      ...
      for (final dartFilePath in allDartFilePathsInModule) {
        ...
        microModuleToLibraryElements[microModule]!.add(maybeResolvedLibraryResult.element);
      }
    }
